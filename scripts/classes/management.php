...
class Management{
  ...
  public function addNewUser($empid, $deptid, $roleid, $username, $pass){
      ...
      if($db = new ezSQL_mysqli(EZSQL_DB_USER,EZSQL_DB_PASSWORD,EZSQL_DB_NAME,EZSQL_DB_HOST)){
        if($db->query("INSERT INTO `empmaster`(`USERID`, `EMPID`, `DEPTID`, `ROLEID`, `OPLOGNO`) VALUES ('$username', '$empid', '$deptid', '$roleid', '999')")){$flag=1;}
        else{$flag=0;}
      }
        else{$flag=0;}
  ...
  public function getUsersList($deptid){
    ...
    $db = new ezSQL_mysqli(EZSQL_DB_USER,EZSQL_DB_PASSWORD,EZSQL_DB_NAME,EZSQL_DB_HOST);
      if($userx = $db->get_results("SELECT USERID, EMPID, ROLEID, EMPFNM, DATEOFBIRTH, GENDER, MOBILENO, OPENDT, CLOSEDT FROM EMPMASTER WHERE DEPTID = '$deptid' AND ROLEID = 3")){
        return $userx;
      }
      else {
        return false;
  ...
  public function addProject($pid, $pnm, $pdesc, $opendt, $closedt){
      //Adds new project
      $date = DateTime::createFromFormat('m/j/Y', $opendt);
      $opendtx = $date->format('Y-m-d');

      $datex = DateTime::createFromFormat('m/j/Y', $closedt);
      $closedtx = $datex->format('Y-m-d');

      $db = new ezSQL_mysqli(EZSQL_DB_USER,EZSQL_DB_PASSWORD,EZSQL_DB_NAME,EZSQL_DB_HOST);
        if($db->query("INSERT INTO `prjctmaster`(`PRJCTID`, `PRJCTNM`, `PRJCTDESCRI`, `OPENDT`, `CLOSEDT`, `OPLOGNO`) VALUES ('$pid','$pnm','$pdesc','$opendtx','$closedtx','999')")){
          return true;
        }
        else {
