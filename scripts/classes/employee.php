...
class Employee
{
    ...
    public function retrieveArtistAll()
    {
        $db = new ezSQL_mysqli(EZSQL_DB_USER, EZSQL_DB_PASSWORD, EZSQL_DB_NAME, EZSQL_DB_HOST);
        if ($userx = $db->get_results("SELECT `EMPID`, `EMPFNM`, `SKILLS` FROM `EMPMASTER` WHERE `ROLEID`= 1 AND `DEPTID`=1001")) {
            return json_encode($userx);
        }
        else{
            throw new Exception('Unable to fetch Artist\'s List');
        }
    }

    public function getArtistAssignBids($id){
        $db = new ezSQL_mysqli(EZSQL_DB_USER, EZSQL_DB_PASSWORD, EZSQL_DB_NAME, EZSQL_DB_HOST);
        if ($bid = $db->get_var("SELECT `BID` FROM `ARTISTBIDS` WHERE `ARTISTID`='$id'")) {
            return $bid;
        }
        else{
            throw new Exception('Artist\'s Bid unable to fetch or does not exists');
        }
    }
}
